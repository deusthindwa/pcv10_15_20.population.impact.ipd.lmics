...
A <-
  ...
  labs(title = "", x = "observed log_number of IPD isolates", y = "pneumococcal serotype") + 
  facet_wrap(.~country) +
...
C <-
  ...
  ungroup() %>%
  ggplot() +
  geom_col(aes(x = reorder(st, -NN), y = log(n+0.5), fill = fct_rev(factor(yearc))), size = 0.3, color = "black", position = "stack") +
  theme_bw(base_size = 16, base_family = "Lato") +
  scale_fill_manual(values = palx) +
  ...
  theme(strip.text.x = element_text(size = 26), strip.background = element_rect(fill = "gray90")) +
  guides(fill = guide_legend(title = "")) +
  theme(legend.text = element_text(size = 14), legend.position = c(0.4, 0.6), legend.title = element_text(size = 14)) +
  theme(panel.border = element_rect(colour = "black", fill = NA, size = 3))
...
ggsave(here("output", "XXX.png"),
       plot = (C), 
       width = 25, height = 15, unit = "in", dpi = 300)
...
D <-
  ...
  labs(title = "", x = "year of sample isolation", y = "proportion of observed vaccine serotype IPD") +
...
#save combined plots
ggsave(here("output", "fig11_ipdsgDist.png"),
       plot = (D),
...
E <-
  ...
  theme_bw(base_size = 16, base_family = "Lato") +
  scale_fill_manual(values = palx) +
  labs(title = "", x = "observed serotype carriage prevalence", y = "pneumococcal serotype") + 
  facet_wrap(.~country) +
...
#save combined plots
ggsave(here("output", "sfig2_carstDist.png"),
...
# 
# #save combined plots
# ggsave(here("output", "fig11_carsgDist.png"),
#        plot = (G),
