...
pcv_carr <-
  bind_cols(
      ...
      mutate(pcv10sii = if_else(grepl("\\b(1|5|6A|6B|7F|9V|14|19A|19F|23F)\\b", st) == TRUE, "cVT", if_else(st == "None", "None", "cNVT")),
             ...
             pcv13pfz = if_else(grepl("\\b(1|3|4|5|6A|6B|7F|9V|14|18C|19A|19F|23F)\\b", st) == TRUE, "cVT", if_else(st == "None", "None", "cNVT")),
  ...
  bind_rows(
    bind_cols(
      ...
      mw_pcvsamples %>%
        ...
        dplyr::mutate(irr1x = 1-irr1, irr2x = 1-irr2, irr3x = 1-irr3) %>%
        dplyr::select(irr1x, irr2x, irr3x)),
...
mw_impactEst <-
  mw_netImpact %>% 
  dplyr::filter(pcv != "pcv10gsk", pcv !="pcv13pfz") %>%
  group_by(pcv) %>%
