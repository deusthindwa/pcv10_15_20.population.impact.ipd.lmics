...
is_impact <-
  bind_rows(
    is_impact %>% dplyr::select(pcv10, country) %>% dplyr::rename("impact" = "pcv10") %>% dplyr::mutate(pcv = "PCV10 vs PCV13"),
    is_impact %>% dplyr::select(pcv15, country) %>% dplyr::rename("impact" = "pcv15") %>% dplyr::mutate(pcv = "PCV15 vs PCV13"),
    is_impact %>% dplyr::select(pcv20, country) %>% dplyr::rename("impact" = "pcv20") %>% dplyr::mutate(pcv = "PCV20 vs PCV13"))
...
sa_impact <-
  bind_rows(
    sa_impact %>% dplyr::select(pcv10, country) %>% dplyr::rename("impact" = "pcv10") %>% dplyr::mutate(pcv = "PCV10 vs PCV13"),
    sa_impact %>% dplyr::select(pcv15, country) %>% dplyr::rename("impact" = "pcv15") %>% dplyr::mutate(pcv = "PCV15 vs PCV13"),
    sa_impact %>% dplyr::select(pcv20, country) %>% dplyr::rename("impact" = "pcv20") %>% dplyr::mutate(pcv = "PCV20 vs PCV13"))
...
mw_impact <-
  bind_rows(
    mw_impact %>% dplyr::select(pcv10, country) %>% dplyr::rename("impact" = "pcv10") %>% dplyr::mutate(pcv = "PCV10 vs PCV13"),
    mw_impact %>% dplyr::select(pcv15, country) %>% dplyr::rename("impact" = "pcv15") %>% dplyr::mutate(pcv = "PCV15 vs PCV13"),
    mw_impact %>% dplyr::select(pcv20, country) %>% dplyr::rename("impact" = "pcv20") %>% dplyr::mutate(pcv = "PCV20 vs PCV13"))
...
all_impactSum <-
  bind_rows(is_impact, sa_impact, mw_impact) %>%
...
A <-
  ggplot() +
  geom_density(data = bind_rows(is_impact, sa_impact, mw_impact), aes(x = impact, group = pcv, fill = country), size = 1, alpha = 0.3) +
  theme_bw(base_size = 16, base_family = "American typewriter") +
  geom_vline(xintercept = 0, linetype = "dashed") +
  scale_y_continuous(limit = c(0, 14.5), breaks = seq(0, 14.5, 3)) +
  ...
  geom_text(data = dplyr::filter(is_impact, pcv == "PCV15 vs PCV13"), aes(x = impact, group = pcv), x = -0.35, y = 10, label = 'cases increase', size = 7, family = "American typewriter",) +
  geom_curve(data = dplyr::filter(is_impact, pcv == "PCV15 vs PCV13"), aes(x = impact, group = pcv), x = -0.05, y = 12, yend = 12, xend = -0.30, linewidth = 2, curvature = 0, arrow = arrow(length = unit(0.7, 'cm'))) +
  geom_text(data = dplyr::filter(is_impact, pcv == "PCV15 vs PCV13"), aes(x = impact, group = pcv), x = 0.35, y = 10, label = 'cases decrease', size = 7, family = "American typewriter",) +
  geom_curve(data = dplyr::filter(is_impact, pcv == "PCV15 vs PCV13"), aes(x = impact, group = pcv), x = 0.05, y = 12, yend = 12, xend = 0.30, linewidth = 2, curvature = 0, arrow = arrow(length = unit(0.7, 'cm')))
